...
class Turbines():
    def __init__(self, master, main_notebook):
        ...
        self.turbines_page = tk.Frame(self.main_notebook, bg='gray20')
            ...
            if len(invalid_entries) == 0:
                ...
                if duplicates_count > 0:
                    ...
                    if confirm_duplicate == 'no':
                        ...
                        # Read from the txt file the highest config id + 1 and insert it with each component
                        ...
                        for x in range(len(self.name_var_list)):
                            ...
                            if self.main_notebook.index(self.main_notebook.select()) == 1:
                                ...
                                if highest_val is None:
                                    ...
                                    c.execute('INSERT INTO ComponentData(id, config_id, type, name, metric, quantity, individual_cost, total_cost) VALUES(1,?,?,?,?,?,?,?)', (CURRENT_CONFIG_ID, _type, self.name_var_list[i].get(), round(self.power_var_list[i].get(), 2), self.quantity_var_list[i].get(), round(self.turbine_cost, 2), round(self.total_turbine_cost, 2)))
                                    db.commit()
                                else:
                                    # Write information to database
                                    c.execute('INSERT INTO ComponentData(id, config_id, type, name, metric, quantity, individual_cost, total_cost) VALUES(?,?,?,?,?,?,?,?)', (highest_val, CURRENT_CONFIG_ID, _type, self.name_var_list[i].get(), round(self.power_var_list[i].get(), 2), self.quantity_var_list[i].get(), round(self.turbine_cost, 2), round(self.total_turbine_cost, 2)))
                                    db.commit()
